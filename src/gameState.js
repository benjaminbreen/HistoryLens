...
export const useGameState = () => {
  ...
  const [gameState, setGameState] = useState({
    ...
    quests: [], // Initialize with the prologue quest
    compounds: [], 
    time: '8:00 AM',  
    date: 'August 22, 1680', 
    location: 'Apothecary shop, Mexico City',
    turnNumber: 1,  // Initialize turn number here
  });
  ...
  const addCompoundToInventory = useCallback((compound) => {
    ...
    // Add or update the compound in the inventory
    setGameState((prevState) => {
      const updatedInventory = prevState.inventory.map((item) => {
        if (item.name.toLowerCase() === compound.name.toLowerCase()) {
          return { ...item, quantity: item.quantity + (compound.quantity || 1) };
        }
        return item;
      });

      const itemExists = updatedInventory.some(item => item.name.toLowerCase() === compound.name.toLowerCase());
      if (!itemExists) {
...
If your response doesn't meet these criteria, please correct it before returning.`;
  ...
  const completeQuest = useCallback((questId, outcome) => {
    setGameState(prevState => {
      ...
      // If you need to modify the inventory based on the quest outcome, do it here
